...
import { eventOperations, playerOperations, reservationOperations, type Event } from '@/lib/db'
...
export function BlockPage() {
  ...
  useEffect(() => {
    const loadInitialEvent = async () => {
      try {
        ...
        setEvent(lastEvent || { id: '', name: '', players: [], created_at: '' })
      } catch (error) {
  ...
  const handleSaveEvent = async (updatedEvent: { name: string, players: string[] }) => {
    try {
      if (event?.id) {
        await eventOperations.updateEvent(event.id, updatedEvent.name, updatedEvent.players)
      } else {
