...
export function PlayerSheet({ player, onSave, trigger }: PlayerSheetProps) {
  ...
  const [cardType, setCardType] = useState<CardType>(player?.cardType || 'Multisport')
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent>
          <SheetHeader>
            ...
            <SheetDescription>Make changes to the player's information here.</SheetDescription>
          </SheetHeader>
            ...
            <div className="flex flex-wrap gap-2">
              {cardTypes.map((type) => (
                <Button
                  ...
                  className={cardType === type ? ' bg-slate-800 text-white ' : 'border-slate-500 text-slate-900 bg-white'}
                >
                  {type}
                </Button>
              ))}
